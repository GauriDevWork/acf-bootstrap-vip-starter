...
$layout = !empty($options['blog_post_layout']) ? $options['blog_post_layout'] : 'grid';
...
<main id="main-content" class="site-main">
	...
	?>
	<section class="{{THEME_PREFIX}}-blog py-75">
		<div class="<?php echo esc_attr({{THEME_PREFIX}}get_container_class()); ?>">
			<div class="{{THEME_PREFIX}}-blog-row">
				<?php

				if ($options['sidebar_position'] == 'left-sidebar') {
					?>
				...
				if ($one->have_posts()):
					...
					<div class="{{THEME_PREFIX}}-blog-card-wrapper">
						<div class="{{THEME_PREFIX}}-blog-card-row <?php echo $class; ?>">
							<?php
							while ($one->have_posts()):
								...
								<div class="{{THEME_PREFIX}}-blog-card">
									<a href="<?php echo get_permalink($post_id); ?>" class="img-block">
										...
										if (!empty($options['post_meta_display']) && in_array('date', $options['post_meta_display'])) {
											?>
									...
									<div class="content-block">
										<?php
										if (!empty($options['post_meta_display']) && in_array('category', $options['post_meta_display'])) {
											if ($categories) {
												?>
				...
				if ($options['sidebar_position'] == 'right-sidebar') {
					?>
					<div class="{{THEME_PREFIX}}-blog-sidebar">
						<?php
						if (is_active_sidebar('blog-sidebar')):
				...
				if ($options['sidebar_position'] == 'no-sidebar' && $class == 'list-view') {
					?>
					<div class="{{THEME_PREFIX}}-blog-sidebar"></div>
					<?php
				}
