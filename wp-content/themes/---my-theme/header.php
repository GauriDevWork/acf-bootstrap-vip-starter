...
<head>
	...
	if (!empty($options['banner_enable'])) {
		...
		if ($banner_bg_image) {
			?>
			<link rel="preload" as="image" href="<?php echo esc_url($banner_bg_image); ?>" fetchpriority="high">
			<?php
		}
	}
...
<body <?php body_class(); ?>>
	...
	$header_fullwidth = $options['header_fullwidth'] == 1 ? 'container-fluid' : esc_attr(my_theme_get_container_class());
				...
				<div class="<?php echo $header_fullwidth; ?>">
					...
					<nav class="navbar ta-navigation-menu">
						...
						<div class="collapse navbar-collapse" id="collapsibleNavbar">
							...
							wp_nav_menu($args);
							if (!empty($options['my_theme_buttons']['text']) && is_array($options['my_theme_buttons']['text'])):
								?>
								<div class="btn-block">
									<?php

									foreach ($options['my_theme_buttons']['text'] as $i => $text) {
										$url = !empty($options['my_theme_buttons']['url'][$i]) ? $options['my_theme_buttons']['url'][$i] : '#';
										$target = !empty($options['my_theme_buttons']['target'][$i]) ? $options['my_theme_buttons']['target'][$i] : '_self';

										echo '<a href="' . esc_url($url) . '" target="' . esc_attr($target) . '" class="my-theme-btn">';
										echo esc_html($text);
										echo '</a>';
									}
									?>
