...
{
	wp_enqueue_script('my-theme-customizer', get_stylesheet_directory_uri() . '/assets/js/customizer.js', array('customize-preview'), _S_VERSION, true);
}
...
add_action('wp_head', 'my_theme_add_custom_favicon');
function my_theme_add_custom_favicon()
{
	$options = get_option('my_theme_options'); // Replace with your option key
...
function my_theme_set_custom_logo_and_favicon()
{
	$options = get_option('my_theme_options');
...
add_action('after_setup_theme', 'my_theme_set_custom_logo_and_favicon');

function my_theme_remove_editor_for_global_template()
{
...
add_action('load-post.php', 'my_theme_remove_editor_for_global_template');
add_action('load-post-new.php', 'my_theme_remove_editor_for_global_template');
...
function my_theme_footer_widgets_init()
{
	...
	register_sidebar(array(
		...
		'id'            => 'footer-1',
		'description'   => esc_html__('Widgets in this area will be shown in the first column of the footer.', 'my-theme'),
	...
	register_sidebar(array(
		...
		'id'            => 'footer-2',
		'description'   => esc_html__('Widgets in this area will be shown in the second column of the footer.', 'my-theme'),
		'before_widget' => '<div id="%1$s" class="widget %2$s">',
		'after_widget'  => '</div>',
		'before_title'  => '<h4 class="widget-title">',
		'after_title'   => '</h4>',
	...
	register_sidebar(array(
		...
		'id'            => 'footer-3',
		'description'   => esc_html__('Widgets in this area will be shown in the third column of the footer.', 'my-theme'),
		'before_widget' => '<div id="%1$s" class="widget %2$s">',
		'after_widget'  => '</div>',
		'before_title'  => '<h4 class="widget-title">',
		'after_title'   => '</h4>',
	...
	// Footer Column 4
	register_sidebar(array(
		'name'          => esc_html__('Footer Column 4', 'my-theme'),
		'id'            => 'footer-4',
		'description'   => esc_html__('Widgets in this area will be shown in the third column of the footer.', 'my-theme'),
		'before_widget' => '<div id="%1$s" class="widget %2$s">',
		'after_widget'  => '</div>',
	...
	// Blog sidebar	
	register_sidebar(array(
		'name'          => esc_html__('Blog Sidebar', 'my-theme'),
		'id'            => 'blog-sidebar',
		'description'   => esc_html__('Widgets in this area will be shown in the blog sidebar.', 'my-theme'),
		'before_widget' => '<div id="%1$s" class="widget %2$s">',
		'after_widget'  => '</div>',
		'before_title'  => '<h4 class="widget-title">',
		'after_title'   => '</h4>',
...
add_filter('body_class', 'my_theme_add_container_class');
function my_theme_add_container_class($classes)
{
	$options = get_option('my_theme_options');
	...
	if ($options['enable_off_canvas']) {
		if ($options['mobile_menu_type'] == 'left') {
			$classes[] = 'off-canvas-left';
		} else if ($options['mobile_menu_type'] == 'right') {
			$classes[] = 'off-canvas-right';
		}
	}

	if ($options['arrow_enable']) {
		$classes[] = '';
	} else {
...
function my_theme_get_container_class()
{
	$options = get_option('my_theme_options');
...
function my_theme_get_excerpt_or_content($length = 50, $post_id = null)
{
...
if (!function_exists('my_theme_get_wp_menus')) {
	function my_theme_get_wp_menus()
	{
		$menus = wp_get_nav_menus();
		$menu_options = array('none' => __('No Menu', 'my-theme'));
...
if (!function_exists('my_theme_get_primary_menu_id')) {
	function my_theme_get_primary_menu_id()
	{
